...
*/
		...
		for (j=AVERAGING_RADIUS; j<WIDTH-AVERAGING_RADIUS - 1; j++) {
			int rest=0, move=0;
			for (i=rad1; i < (HEIGHT / splits) - rad2 - 1; i++ ) {
	...
	void c_iprint(int a, int n, int j, int k )//InputDF &fi
	{
	...
	{
		//printf("Source_S_%d_%d_%d\n", split, iter, ens);
		//fflush(stdout);
		char *field = fi.getData<char>();
		df = fi;
		if (iter == 1){
			fillS(field, 0, WIDTH/2-SOURCE_WIDTH/2, HEIGHT, WIDTH/2+SOURCE_WIDTH/2, 1, 1, 1, 1, 0.75, 0, 0, 0, split, iter, ens);
			//printf("%d\t%d\t%d\t%d\t%d\t%d\n", split, iter, (HEIGHT / splits) * (split - 1), WIDTH/2-SOURCE_WIDTH/2, (HEIGHT / splits) * split, WIDTH/2+SOURCE_WIDTH/2);
			//fflush(stdout);
		}
		//printf("Source_E_%d_%d_%d\n", split, iter, ens);
		//fflush(stdout);
		/*
		char field_path[100];
		FILE *fl_field;
